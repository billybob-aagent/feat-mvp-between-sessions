...
export default function LibraryItemDetailPage() {
  ...
  const sections = useMemo(() => {
    if (!item) return [];
    return Array.isArray(item.sections) ? item.sections : item.sections?.sections ?? [];
  }, [item]);
  ...
  async function requestSignature() {
    ...
    try {
      const response = (await apiFetch(`/library/forms/${itemId}/signature-requests`, {
        method: "POST",
        json: { clientId: selectedClient },
      })) as { pdfSnapshotRef: string };
