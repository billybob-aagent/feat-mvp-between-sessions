...
import { clinicListAssignments } from "@/lib/clinic-api";
import { ClinicAssignmentListItem } from "@/lib/types/clinic";
...
export default function ClinicAssignmentsPage() {
  ...
  const [items, setItems] = useState<ClinicAssignmentListItem[]>([]);
  ...
  const loadFirstPage = useCallback(async () => {
    ...
    setStatus(null);
    try {
      const data = await clinicListAssignments({
      ...
      <div className="flex items-start justify-between gap-3 mb-6">
        <div>
          ...
          <p className="text-sm text-app-muted mt-1">
            Published and draft assignments across the clinic.
          </p>
        </div>
