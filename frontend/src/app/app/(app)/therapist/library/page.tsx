...
export default function TherapistLibraryPage() {
  ...
  const metadataOptions = useMemo(() => {
    ...
    for (const item of items) {
      const meta: LibraryMetadata = item.metadata ?? {};
      (meta.primaryClinicalDomains || []).forEach((value) => domains.add(value));
      (meta.applicableModalities || []).forEach((value) => modalities.add(value));
      (meta.targetPopulation || []).forEach((value) => populations.add(value));
