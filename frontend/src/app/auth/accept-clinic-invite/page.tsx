...
export default function AcceptClinicInvitePage() {
    ...
    async function load() {
      ...
      try {
        ...
        const data = await apiFetch<InviteLookup>(
          `/auth/clinic-invite?token=${encodeURIComponent(token)}`,
        );
        if (cancelled) return;
        setInvite(data);
      } catch (e) {
  ...
  async function submit(e: React.FormEvent) {
    ...
    try {
      await apiFetch("/auth/register/clinic-therapist", {
        method: "POST",
        json: {
          token,
          fullName,
  ...
  return (
    <main className="min-h-screen bg-app-bg text-app-text">
      <div className="max-w-md mx-auto px-6 py-16">
        <Card>
          <CardHeader>
            <CardTitle>Accept clinic invite</CardTitle>
            <p className="text-sm text-app-muted">
              Set up your therapist access for this clinic.
            </p>
          </CardHeader>
          <CardContent>
            {invite && (
            ...
            <form onSubmit={submit} className="space-y-4">
              ...
              <div>
                <label className="text-label text-app-muted">Organization (optional)</label>
                <Input
              ...
              <div>
                <label className="text-label text-app-muted">Timezone (optional)</label>
                <Input
                  placeholder="UTC"
                  value={timezone}
