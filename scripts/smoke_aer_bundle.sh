...
COOKIE_JAR="${COOKIE_JAR:-/tmp/bs-cookie-aer-bundle.txt}"
...
echo "== Step 3: request bundle"
BUNDLE_FILE=$(mktemp)
BUNDLE_HEADERS=$(mktemp)
curl -s -D "${BUNDLE_HEADERS}" -o "${BUNDLE_FILE}" \
  -b "${COOKIE_JAR}" \
  -H "Content-Type: application/json" \
  -X POST "${BASE_URL}/reports/aer-bundle" \
  -d "{\"clinicId\":\"${CLINIC_ID}\",\"clientId\":\"${CLIENT_ID}\",\"start\":\"${START}\",\"end\":\"${END}\"}"
...
if [[ ! -f "${TMP_DIR}/AER.json" || ! -f "${TMP_DIR}/AER.pdf" || ! -f "${TMP_DIR}/verification.txt" ]]; then
  echo "Bundle missing expected files."
  exit 1
fi
...
echo "PDF_SHA256=${PDF_HASH}"

if [[ "${JSON_HASH}" != "${EXPECTED_JSON}" ]]; then
  echo "JSON hash mismatch (verification.txt)."; exit 1; fi
if [[ "${PDF_HASH}" != "${EXPECTED_PDF}" ]]; then
  echo "PDF hash mismatch (verification.txt)."; exit 1; fi
