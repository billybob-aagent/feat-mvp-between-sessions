...
node -e "const fs=require('fs'); const addFormats=require('./backend/node_modules/ajv-formats'); const Ajv=(()=>{ try { return require('./backend/node_modules/ajv/dist/2020'); } catch (err) { return require('./backend/node_modules/ajv'); } })(); const schema=JSON.parse(fs.readFileSync('${SCHEMA_PATH}','utf8')); const data=JSON.parse(fs.readFileSync('${json1}','utf8')); const ajv=new Ajv({allErrors:true,strict:true}); addFormats(ajv); const validate=ajv.compile(schema); if(!validate(data)){ console.error(validate.errors); process.exit(1);}"
