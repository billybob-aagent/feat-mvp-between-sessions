...
const envOrArg = (key: string, fallback: string) => {
  if (argMap.has(key)) return argMap.get(key) as string;
...
const buildInternalUrl = (ext: string) => {
...
const buildExternalUrl = (ext: string, tokenValue: string) => {
